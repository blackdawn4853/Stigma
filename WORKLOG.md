...
## 2026-06-24 — [델노트북] 프로젝트를 OneDrive → 로컬로 이전

- **증상**: 노트북에서 에셋 일부 누락 + 전투 시 몬스터 0마리.
- **원인**: 프로젝트가 OneDrive 폴더(`OneDrive\문서\GitHub\Stigma`) 안에 있어서, OneDrive 용량 초과로 파일들이 "클라우드 전용 placeholder(빈 껍데기)"로 남아 유니티가 제대로 못 읽었음.
- **조치**: GitHub에서 **`C:\Dev\Stigma`(OneDrive 바깥)로 새로 clone** → 정상 가동 확인. (참고: 노트북 작업 폴더는 이제 여기. 데탑은 기존 경로 그대로, GitHub로 동기화 동일.)
- ⚠️ **유니티 프로젝트는 절대 OneDrive 안에 두지 말 것.**
